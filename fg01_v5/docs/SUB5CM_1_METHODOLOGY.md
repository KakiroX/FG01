...
> **Superseded-economics notice.** Section 7.1 below describes engagement rate
> as "derived rather than assumed" from cone-entry/J2 enumeration, but the
> uncertainty quantification in Section 8 of the companion results document
> ([SUB5CM_2_RESULTS.md](SUB5CM_2_RESULTS.md)) still samples engagements per
> platform lifetime as a free parameter over a wide range (including the
> highlighted 10,000-engagement case), rather than fixing it in closed form.
> That treatment is superseded by ECO-1, which derives achievable engagement
> rate directly from orbital mechanics (Δv per engagement =
> π·v_orbital/(3.5·Ω̇·T_mission)) and finds it fixed at approximately 24–55 per
